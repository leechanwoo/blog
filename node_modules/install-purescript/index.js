...
const {constants: {BROTLI_PARAM_SIZE_HINT}, brotliCompress, createBrotliDecompress, gzip} = require('zlib');
	...
	return new Observable(observer => {
		...
		function main({brokenCacheFound = false} = {}) {
			...
			runInDir(cwd, () => subscriptions.add(downloadOrBuildPurescript(options).subscribe({
				...
				async complete() {
					const writeCacheValue = {
						id: 'write-cache',
						brotli: !!BROTLI_PARAM_SIZE_HINT
					};
					const tarBuffers = [];
					...
					try {
						...
						const decomressed = await promisify(brotliCompress || gzip)(Buffer.concat(tarBuffers, tarSize), BROTLI_PARAM_SIZE_HINT ? {
							params: {
		...
		(async () => {
			...
			try {
				...
				await promisify(pump)(createReadStream(cachePath), ...createBrotliDecompress ? [createBrotliDecompress()] : [], new Unpack({
					strict: true,
					cwd,
					filter(_, entry) {
						entry.path = binName;
